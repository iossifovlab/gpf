...
EOT'
  ...
  build_stage "Tests - dae"
  {
    ...
    build_run_container bash -c '
        ...
        /opt/conda/bin/conda run --no-capture-output -n gpf py.test -v --no-cleanup -n 10 --durations 20 \
  ...
  build_stage "Tests - wdae"
  {
    ...
    build_run_container bash -c '
        cd /code/wdae;
        export PYTHONHASHSEED=0;
        /opt/conda/bin/conda run --no-capture-output -n gpf py.test --no-cleanup -v -n 10 --durations 20 \
          --cov-config /code/coveragerc \
