...
for d in /wd/gain_core /wd/gain_spliceai_annotator; do
    cd ${d};
...
/opt/conda/bin/conda run --no-capture-output -n gpf \
    py.test -vv -s --log-level=DEBUG \
        ...
        /wd/gain_spliceai_annotator/tests

/opt/conda/bin/conda run -n gpf \
    coverage xml

cp /wd/gain_spliceai_annotator/coverage.xml /wd/test-results/

/opt/conda/bin/conda run -n gpf \
    coverage html --title gain_spliceai_annotator -d /wd/test-results/coverage-html
