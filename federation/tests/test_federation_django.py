...
def test_families_tags_download(
    ...
    assert len(res) == 9
    assert res[0] == (
        b"familyId\tpersonId\tmomId\tdadId\tsex\tstatus\trole\tsample_id\tlayout\tg"
        b"enerated\tnot_sequenced\ttag_nuclear_family\ttag_quad_family\ttag_trio_famil"
        b"y\ttag_simplex_family\ttag_multiplex_family\ttag_control_family\ttag_affecte"
        b"d_dad_family\ttag_affected_mom_family\ttag_affected_prb_family\ttag_affecte"
        b"d_sib_family\ttag_unaffected_dad_family\ttag_unaffected_mom_family\ttag_una"
        b"ffected_prb_family\ttag_unaffected_sib_family\ttag_male_prb_family\ttag_fem"
        b"ale_prb_family\ttag_missing_mom_family\ttag_missing_dad_family\tmember_inde"
        b"x\tphenotype"
    )
    assert res[1] == (
        b"f1.1\tmom1\t0\t0\tF\tunaffected\tmom\tmom1\t1:10.0,50.0\tFalse\tFalse\tTrue"
        b"\tTrue\tFalse\tTrue\tFalse\tFalse\tFalse\tFalse\tTrue\tFalse\tTrue\tTrue\tFa"
        b"lse\tTrue\tFalse\tTrue\tFalse\tFalse\t0\tunaffected"
    )
