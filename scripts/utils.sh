...
function create_docker_network {
    ...
    then
        ...
        DOCKER_NETWORK_ARG="--network ${NETWORK}"
    fi
...
function run_gpf_impala {
    ...
    export HAS_GPF_IMPALA=`docker ps | grep ${CONTAINER_GPF_IMPALA} | sed -e "s/\s\{2,\}/\t/g" | cut -f 1`
...
function run_gpf_remote {
    ...
    export HAS_GPF_REMOTE=`docker ps | grep ${CONTAINER_GPF_REMOTE} | sed -e "s/\s\{2,\}/\t/g" | cut -f 1`

    if [[ -z $HAS_GPF_REMOTE ]];
    then
        echo "Going to create GPF remote container ${CONTAINER_GPF_REMOTE}..."
        docker run \
            -d --rm ${DOCKER_NETWORK_ARG} \
