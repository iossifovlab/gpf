...
class StudyFilters(object):
    ...
    STUDY_NAMES = set(st.name for st in STUDIES)
...
class RaceFilter(FamilyFilter):
    ...
    def get_matching_probands_by_race(self, race):
        df = self.phdb.get_measure_values_df('pheno_common.race', role='prb')
        df = df[df['pheno_common.race'] == race]

        person_ids = set(df.person_id.values)
        return person_ids

    def filter_matching_probands_by_race(self, race, probands):
        filter_probands = self.get_matching_probands_by_race(race)
        return probands & filter_probands
