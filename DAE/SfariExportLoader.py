    ...
    def open(self):
        ...
        if self.fileOpen == False:        
            ...
            for row in self.variablesNode.iterrows():                    
                ...
                self.variableTracker.add(row['variableName'],row['members'],row['source'])   
                                        
    def addGroup(self, name):
        if self.fileOpen == True:
            group = self.handle.createGroup("/", name, name)            
            self.handle.flush()  
...
class SfariExportLoader
    ...
    This class creates the HDf5 version of the SFARI base data. This class does alot
    but it is really meant to be run once.  Its purpose is to load the CSV SFARI files,
    imput the variable metadata information, create a H5 file.  But it also returns
    the data structures dt and md that Ivan Iossifov described.  This is class can be used to
    return the non-H5 version for dt and md.
...
methods:
    ...
    def loadFile(self, filename, rowsToLoad=-1, startOfVariables=17, includeEmptyValues=False):
...
class SfariExportLoader:
    ...
    # This function can be called more than once to load multiple SFARIBase export files.                            
            ...
            if idx == 0:      
                ...
                for column in row:
                    ...
                    if i < startOfVariables:  
                        ...
                        columnsMember[column]['variable'] = column
                        columnsMember[column]['member'] = self.variableTracker.translate(column[0:1])
                        columnsMember[column]['source'] =  basename                                        
...
#                             
        ...
        for key in self.variableTracker.variables.keys():            
            ...
            for familyId in self.data:
                ...
                if variableName not in variables:                             
                    variables[variableName] = {'datatype':{'empty':0,'numeric':0,'float':0,'integer':0,'boolean':0,'date':0,'string':0},
                                               'values':{},
                                               'length':0}
            ...
            if variableName not in variables:
                ...
                # it is empty
                variables[variableName] = {'datatype':{'empty':1,'numeric':0,'float':0,'integer':0,'boolean':0,'date':0,'string':0},
                                           'values':{},
                                           'length':0}         
                md[variableName] = {'datatype':'unknown','values':{},'uniqueValues':0,'members':{}}                
    ...
    def getValueArray(self,variableName,member):
        ...
        #vls = [ float(dt[x]['variables'][variablename])  for x in dt.keys() if variablename in dt[x]['variables'] and dt[x]['member'] == "p1" ]        
        #vls = [ float(self.data[x][4])  for x in variableIndex if self.data[x][2] == "p1" ]
        vls=[]
        for x in variableIndex:
            if self.data[x][2] == member:
                try:
    ...
    def saveh5(self,h5filename, title):
        ...
        earray = h5File.createEArray("/", nodeName, h5dataType, (0,), expectedrows=numberOfRows, filters=myfilter)
        earray.append(families)        

        metaDataGroup = h5File.createGroup("/", 'metadata', 'metadata')        
        numberOfVariables = self.variableTracker.count()                                
        variablesTable = h5File.createTable(metaDataGroup, 'variables', VariablesTableParticle, "variables")
        ...
        for key in sorted(self.variableTracker.variableNames()):                                    
            ...
            #name = name.replace(".", "_")                                        
            if dataType=="text" or dataType=="string" or dataType=="label":                       
                ...
                msg="Hdf5TableGroup handler write does not support the type of %s %s."%(variableName,dataType)
                raise TypeError(msg)
                            
            earray = h5File.createEArray("/", variableName, h5dataType, (0,), expectedrows=numberOfRows, filters=myfilter)                
            ...
            for familyId in self.families:
                ...
                if dataType=='integer':
                    try:
                        ...
                        #print "ValueError error int ", variableName, ", ", variableValue, ",", sys.exc_info()[0]
                        variableValue = None                                   
                elif dataType=='long':
                    try:
                        variableValue=int(variableValue)
                    except ValueError:
                        #print "ValueError error float ", variableName, ", ", variableValue, ",", sys.exc_info()[0]
                        variableValue = None                                                       
                elif dataType=='float' or  dataType=='numeric':
                    try:
            ...
            try:
                ...
                print "ValueError error with ", variableName, ", ", sys.exc_info()[0]    
    ...
    def saveh5Tree(self,h5filename, title):
        ...
        nodeName='families'    
        h5dataType=tables.StringAtom(itemsize=8)
        earray = h5File.createEArray("/", nodeName, h5dataType, (0,), expectedrows=numberOfRows)
        earray.append(families)        
        ...
        for member in self.members:
            ...
            nodeName='personId'
            h5dataType=tables.StringAtom(itemsize=8)
            earray = h5File.createEArray(membergroups[member], nodeName, h5dataType, (0,), expectedrows=numberOfRows)
        ...
        for key in self.variables:        
            ...
            for member in members:
                ...
                if dataType=="text" or dataType=="string" or dataType=="label":                       
                    ...
                    msg="Hdf5TableGroup handler write does not support the type of %s."%(dataType)
                    raise TypeError(msg)
                                
                earray = h5File.createEArray(membergroups[member], variableName, h5dataType, (0,), expectedrows=numberOfRows)
                ...
                for individual in individuals:    
                    ...
                    if variableName in self.data[individual]['variables']:                                                                                            
                        variableValue = self.data[individual]['variables'][variableName]  
                        
                        if dataType=='integer':
                            try:
                                variableValue=int(variableValue)
                            except ValueError:
                                print >> sys.stderr, "ValueError error int ", variableName, ", ", variableValue, ",", sys.exc_info()[0]
                                variableValue = None                                   
                        elif dataType=='long':
                            try:
                                variableValue=int(variableValue)
                            except ValueError:
                                print >> sys.stderr, "ValueError error float ", variableName, ", ", variableValue, ",", sys.exc_info()[0]
                                variableValue = None                                                       
                        elif dataType=='float' or  dataType=='numeric':
                            try:
                                variableValue=float(variableValue)
                ...
                try:
                    ...
                    print >> sys.stderr,"ValueError error with ", variableName, ", ", sys.exc_info()[0]    
