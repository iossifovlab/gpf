...
# family,id(),sex,father,mother,birth,guid,individual_properties.role,individual_properties.genetic_ab,individual_properties.genetic_ab_notes,family.collection
    ...
    def _loadSample(self):
        f = open(
            os.path.join(self.sfariDir,
                         'RUCDR_ID-Portal_ID-Map_20111130-id.csv'))
        _h = f.readline()
    ...
    def _loadTwins(self):
        ...
        _h = f.readline()
        # Family,Proband Code,Twin Code,Twin Study
        # Role,Zygosity,Concordance,Collection
        collectionMap = {
            'Simons Twins Collection': 'stc',
        ...
        for l in f:
            ...
            (familyId, prbCode, twnCodesS, twnRolesS,
             zygosity, _concordance, collection) = cs
            ...
            for cd, rl in zip(codes, roles):
                ...
                if p.familyId != familyId or p.role != rl or \
                        p.collection != tg.collection:
                    raise Exception(
                        "The person info from the twind file doesn't "
                        "match the one in the individual file")
                tg.twins.add(p)
