...
def test_query_with_pheno_filters_work(quads_f1_wrapper, pheno_query):
    variants = quads_f1_wrapper.get_variants(phenoFilters=pheno_query)
...
def test_query_with_pheno_filters_and_people_ids_filter(quads_f1_wrapper):
    pheno_query = [FILTER_QUERY_CONTINUOUS]
    variants = quads_f1_wrapper\
        .get_variants(phenoFilters=pheno_query, people_ids=['mom1'])
    variants = list(variants)

    assert len(variants) == 0


def test_query_with_bogus_pheno_filters_raises(quads_f1_wrapper):
