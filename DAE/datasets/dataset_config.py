...
class DatasetConfig(ConfigurableEntityConfig):
    ...
    @staticmethod
    def _get_list_options(options):
        list_options = set([e for el in options if el is not None for e in el])

        return list_options if len(list_options) != 0 else None

    @staticmethod
    def _get_bool_options(options):
        bool_options = any(options)

        return bool_options

    @classmethod
    def _extract_study_config_options(cls, dataset_config, study_configs,
                                      study_config_options):
        study_config_options_list = {
            'studyTypes': None
        }
        study_config_options_bool = {
            'genotypeBrowser': [
        ...
        for study_config in study_configs:
            for option in study_config_options.keys():
                if type(study_config_options[option]) is dict:
                    for opt in study_config_options[option].keys():
                        if option not in study_config_options_list and\
                                option not in study_config_options_bool:
                            continue
                        if option in study_config_options_list:
                            if opt in study_config_options_list[option]:
                                dataset_config[option][opt] =\
                                    cls._get_list_options(
                                        study_config_options[option][opt])
                        elif option in study_config_options_bool:
                            if opt in study_config_options_bool[option]:
                                dataset_config[option][opt] =\
                                    cls._get_bool_options(
                                        study_config_options[option][opt])
                else:
                    if option in study_config_options_list:
                        dataset_config[option] =\
                            cls._get_list_options(study_config_options[option])
                    elif option in study_config_options_bool:
                        dataset_config[option] =\
                            cls._get_bool_options(study_config_options[option])

        dataset_config['genotypeBrowser']['hasStudyTypes'] =\
            False if dataset_config['studyTypes'] is None else True

        return dataset_config

    @classmethod
    def _get_study_config_options(
            cls, dataset_config, study_group_factory, study_group_definition):
        study_config_options = {
            'studyTypes': [],
            'genotypeBrowser': {
                'hasComplex': [],
        ...
        study_configs = study_group_factory.get_study_configs(
            study_group_definition.get_study_group_config(
        ...
        for study_config in study_configs:
            for option in study_config_options.keys():
                if type(study_config_options[option]) is dict:
                    for opt in study_config_options[option].keys():
                        study_config_options[option][opt].append(
                            study_config[opt])
                else:
                    study_config_options[option].append(study_config[option])

        dataset_config = cls._extract_study_config_options(
            dataset_config, study_configs, study_config_options)

        return dataset_config

    def get_dataset_description(
            self, study_group_factory, study_group_definition):
        keys = DatasetConfig._get_dataset_description_keys()
        dataset_config = self.to_dict()

        dataset_config = DatasetConfig._get_study_config_options(
            dataset_config, study_group_factory, study_group_definition)
