...
from pheno.prepare.individuals2ped import FamilyToPedigree
from pheno.tests.test_ped2individuals import BaseFamily
...
class BaseIndividuals2PedFamily(BaseFamily):
    ...
    def test_relations(self, relations_to_test, family_roles, family2ped):
        members = family2ped.to_pedigree(
            self.family_members(family_roles))

        assert_mother_father_by_individual_id(
            members, relations_to_test[0], relations_to_test[1],
            relations_to_test[2])
...
class TestExtendedFamily(BaseIndividuals2PedFamily):
    ...
    SIMPLE_RELATIONS_TO_TEST = {
        FamilyIds.DAD:
            [FamilyIds.PATERNAL_GRANDMOTHER, FamilyIds.PATERNAL_GRANDFATHER],
        FamilyIds.MOM:
            [FamilyIds.MATERNAL_GRANDMOTHER, FamilyIds.MATERNAL_GRANDFATHER],
        FamilyIds.SIB: [FamilyIds.MOM, FamilyIds.DAD],
        FamilyIds.PRB: [FamilyIds.MOM, FamilyIds.DAD],
    ...
    def test_ambiguous_roles_give_warning(
        ...
        family2ped.to_pedigree(
            self.family_members(family_roles))

        assert mock_stderr.write.call_count == \
