...
def fill_data(d_all, n_all, study_name, denovo_study_name):
    ...
    for v in denovo_study.get_denovo_variants():
        if (v.atts['StudyName'] == denovo_study_name or
                v.atts['StudyName'] == 'Yuen2016'):
            d_all.append(form_v(
                [v.atts['loc'], v.atts['var'], v.atts['SampleID']]))

    for v in new_study.get_denovo_variants():
        if ',' in v.atts['sampleIds']:
            for sample_id in v.atts['sampleIds'].split(', '):
                n_all.append(form_v(
                    [v.atts['location'], v.atts['variant'], + sample_id]))
        else:
            n_all.append(form_v(
                [v.atts['location'], v.atts['variant'], v.atts['sampleIds']]))
...
def status(d_all, n_all, study_name):
    print('denovo: {}, {}: {}'.format(
        len(d_all), study_name, len(n_all)), file=sys.stdout)
