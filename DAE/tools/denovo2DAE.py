...
def special_case(person):
    if person.role not in ROLES:
        if person.personId in person_variant:
            ...
            UNCOMMON_FAMILY_MEMBERS.append(person) #TODO export later
...
def assign_family(grouped):
    for family_id, family_df in grouped:
        ...
        for _, person in family_df.iterrows():
            if special_case(person): continue

            id, role = person.personId, person.role
            person_gender[id] = person.gender
            person_family[id] = family_id

            if role == 'mom': f['mom'] = [id]
            elif role == 'dad': f['dad'] = [id]
            elif role == 'prb': append_val('prb', id, f)
            else: append_val('sib', id, f)
...
def find_prob_family(probant_id):
    try: return person_family[probant_id]
    except: return None
    ...
    for role in ROLES:
        if role in family:
            for member_id in family[role]:
                if member_id in person_variant and variant in person_variant[member_id]:
                    best_state.append('1')
...
def generate_dae(variants, families, zb):
    ...
    for _, row in variants.iterrows():
        counter += 1
        if counter % 1000 == 0: print('{} lines processed'.format(counter), file=sys.stdout)

        family_id = find_prob_family(row.sampleId)
        if not family_id: continue

        bs, ch, id = stats(form_variant(row), families_dict[family_id])
        pos = row.pos + 1 if zb else row.pos
        yield [family_id, row.chr, pos, row.ref, row.alt, bs, ch, id]
...
def ref_alt_nan_correction(v):
    ...
    for index,row in v.iterrows():
        ...
        if row.ref is pd.np.nan:
            new_ref = genome.getSequence(row.chr, row.pos, row.pos+len(row.alt)-1)
...
def perform_corrections(v):
    if v.ref.isnull().any() or v.alt.isnull().any():
        ...
        print('>>> {} variants changed.'.format(l), file=sys.stdout)

    print('Grouping consecutive substitutions...', file=sys.stdout)
    l,v = len(v),consecutive_subs_correction(v)
    print('>>> {} new complex variants formed.'.format(l-len(v)), file=sys.stdout)
...
def denovo2DAE(args):
    ...
    prepared_data = pd.DataFrame(generate_dae(v,f, args['zerobased']), columns=
