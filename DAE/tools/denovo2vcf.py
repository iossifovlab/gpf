...
def get_argument_parser():
    ...
    parser.add_argument(
        'pedigree', type=str, metavar='<pedigree filename>',
        help='families file in pedigree format')
    parser.add_argument(
        'denovo', type=str, metavar='<denovo filename>',
        help='denovo file to import')
    parser.add_argument(
        '-o', '--out', type=str, default='./output.vcf', dest='out',
        metavar='output filepath',
        help='output filepath. If none specified, output file is output.vcf')
    parser.add_argument(
        '--pedigree-delimiter', help='delimiter used in the split column in '
        'pedigree; defaults to "\\t"', default='\t', action='store')
    parser.add_argument(
        '--denovo-delimiter', help='delimiter used in the split column in '
        'denovo; defaults to "\\t"', default='\t', action='store')
    parser.add_argument(
        '--pedigree-personId', help='personId column name used in pedigree; '
        'defaults to "personId"', default='personId', action='store')
    parser.add_argument(
        '--denovo-personId', help='personId column name used in denovo; '
        'defaults to "personId"', default='personId', action='store')
    parser.add_argument(
        '--denovo-chromosome', help='chromosome column name used in denovo; '
        'defaults to "chrom"', default='chrom', action='store')
    parser.add_argument(
        '--denovo-position', help='position column name used in denovo; '
        'defaults to "pos"', default='pos', action='store')
    parser.add_argument(
        '--skip-columns', help='Comma separated columns from denovo to skip',
        default=None, action='store')
    parser.add_argument(
        '--sort-order', help='Order for sorting variants', default=None,
        action='store')
...
class DenovoToVcf(object):
    ...
    def __init__(self, pedigree_file, denovo_file, vcf_file, pedigree_personId,
                 denovo_personId, pedigree_delimiter, denovo_delimiter):
    ...
    def convert(self, chromosome, position, skip_columns=[], sort_order=[]):
        ...
        with open(self.input_filename, "r") as input_file:
            ...
            fieldnames = [fieldname for fieldname in reader.fieldnames
                          if fieldname not in skip_columns] + self.peopleIds

            fieldnames.remove('FILTER')

            fieldnames = fieldnames[:5] +\
                ['QUAL', 'FILTER', 'INFO', 'FORMAT'] + fieldnames[5:]
...
def main():
    ...
    converter = DenovoToVcf(
        args.pedigree, args.denovo, args.out, args.pedigree_personId,
        args.denovo_personId, args.pedigree_delimiter, args.denovo_delimiter)
    converter.convert(
