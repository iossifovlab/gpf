...
dir_setup() {
    data_tar_time=`stat -c %Y data-hg19-startup-3.0.0dev-genotype-storage.tar.gz`
    data_dir_time=`stat -c %Y data-hg19-startup`
    if [ ! -d 'data-hg19-startup' ] || [ $data_tar_time -nt $data_dir_time ];
    then
        tar zxf data-hg19-startup-3.0.0dev-genotype-storage.tar.gz
    fi
}
...
if [ -f 'data-hg19-startup-3.0.0dev-genotype-storage.tar.gz' ];
then
    ...
    if [ $? != 0 ];
    then
        rm data-hg19-startup-3.0.0dev-genotype-storage.tar.gz
	get_file
    fi
else
    get_file
