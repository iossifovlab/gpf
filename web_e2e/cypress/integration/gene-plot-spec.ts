...
describe.skip('Gene plot visual tests', () => {
  ...
  it('should condense introns', () => {
    ...
    cy.get('g#plot').matchImageSnapshot('gene-plot/not-condensed-introns');

    page.condenseIntronsCheckbox.click();
    cy.get('g#plot').matchImageSnapshot('gene-plot/condensed-introns');
  });

  it('should compare visually TTN gene plot results', () => {
    geneBrowserPage.searchInputBox.type('ttn');
    geneBrowserPage.goButton.click();
    cy.get('g#plot').matchImageSnapshot('gene-plot/ttn-gene-plot-snapshot');

    cy.get('gpf-table').matchImageSnapshot('gene-plot/ttn-gene-table-snapshot');
  });
});
