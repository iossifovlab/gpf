...
test.describe('Pheno browser tests', () => {
  test.beforeEach(async({ page }) => {
    ...
    await utils.navigateToDatasetPage(page, utils.datasetIds.compAllLiftover, 'Phenotype browser');
  });
  ...
  test('should have working table header sorting buttons', async({ page }) => {
    ...
    await expect(page.locator('.measure-name-cell').nth(0)).toHaveText('m5');
  ...
  test('should download all instruments and validate whether they are equal to the reference data', async({ page }) => {
    ...
    const fixtureData = scanCSV('playwright/fixtures/pheno-browser/measures_comp_all.csv', {sep: ',', nullValues: '-'});
