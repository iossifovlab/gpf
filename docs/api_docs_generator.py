...
def collect_docstrings(route):
    ...
    url_tree = ast.parse(pathlib.Path(urls_filepath).read_text())

    views_filepath = os.path.join(route, "views.py")
    views_tree = ast.parse(pathlib.Path(views_filepath).read_text())
...
def main(argv=None):
    ...
    for view_dir in view_dir_list:
        ...
        try:
            ...
            logger.warning("Failed to collect docstrings from %s", api_name)
        else:
