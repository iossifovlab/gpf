...
def compare(folderA,folderB):
    ...
    for i in range(1,numberOfCmdsA+1):
        ...
        elif exitCodesA[i]!='1' and exitCodesB[i]=='0':
            row.append('1_FAIL')

        elif exitCodesA[i]=='0' and exitCodesB[i]!='1':
            row.append('2_FAIL')
...
def usage(exitCode=0):
    print 'compareUT.py -a <folderA> -b <folderB>'
    sys.exit(exitCode)
...
def main(argv):
    ...
    if folderA == None:
        usage(2)
        #folderA="/data/safe/leotta/unittest/production"


    if folderB == None:
        usage(2)
        #folderB="/data/safe/leotta/unittest/development"
