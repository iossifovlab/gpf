...
def _create_verif_email(host, path, verification_path):
	settings = {
		...
		'initial_message': 'Hello. Follow this link to validate your account and to set your new password: ',
		'host': host,
		'path': path,
...
def _create_reset_mail(host, path, verification_path):
	settings = {
		...
		'initial_message': 'Hello. To change your password please follow this link: ',
		'host': host,
		'path': path,
		'verification_path': verification_path
	}
...
''' settings[dict] must contain : subject, initial_message, host, path, verification_path'''
def _build_email_template(settings):
	subject = settings['subject']
	message = settings['initial_message']
	message += '{0}{1}{2}'.format(settings['host'], settings['path'], settings['verification_path'])
