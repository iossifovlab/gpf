...
'''
import unittest
from api.reports.variants import StudyVariantReports
...
class Test(unittest.TestCase):
    ...
    def test_deserialize_family_reports_family_counters(self):
        ...
        (_p, c) = fc.get_counter('prbMsibF')
        self.assertEqual(877, c)
        (_p, c) = fc.get_counter('prbMsibM')
        self.assertEqual(790, c)
        (_p, c) = fc.get_counter('prbM')
        self.assertEqual(500, c)
        (_p, c) = fc.get_counter('prbFsibF')
        self.assertEqual(128, c)
        (_p, c) = fc.get_counter('prbFsibM')
        self.assertEqual(107, c)
        (_p, c) = fc.get_counter('prbF')
        self.assertEqual(106, c)
        (_p, c) = fc.get_counter('sibF')
        self.assertEqual(6, c)
        (_p, c) = fc.get_counter('sibM')
