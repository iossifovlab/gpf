...
def test_admin_can_remove_password_of_user(admin_client, active_user):
    assert active_user.has_usable_password()

    url = '/api/v3/users/{}/password_remove'.format(active_user.pk)
...
def test_removing_user_password_deauthenticates_them(
    ...
    remove_password_url = '/api/v3/users/{}/password_remove'.format(user.pk)

    response = admin_client.post(remove_password_url)
    assert response.status_code is status.HTTP_204_NO_CONTENT
...
def test_user_cant_remove_other_user_password(user_client, active_user):
    assert active_user.has_usable_password()
...
def test_anonymous_user_cant_remove_other_user_password(client, active_user):
    assert active_user.has_usable_password()
...
def test_inactive_user_stays_inactive(admin_client, inactive_user):
    assert not inactive_user.has_usable_password()

    url = '/api/v3/users/{}/password_remove'.format(inactive_user.pk)
    response = admin_client.post(url)
