...
@pytest.fixture
def project_fixture(tmp_path_factory: TempPathFactory) -> ImportProject:
...
def test_import_project_chromosomes_simple(project_fixture: ImportProject) -> None:
    assert project_fixture.get_variant_loader_chromosomes() == ["chr1", "chr2"]


def test_import_project_variant_loader_types(project_fixture: ImportProject) -> None:
