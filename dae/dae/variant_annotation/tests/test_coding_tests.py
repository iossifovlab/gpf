...
from dae.variant_annotation.effect_checkers.coding \
    import CodingEffectChecker
...
def positive_strand(request):
    ...
    exons = [ExonMock(65, 70, 0),
             ExonMock(80, 90, 1),
             ExonMock(100, 110, 2)]
    request.cls.tm = TranscriptModelMock("+", 1, 2000, exons, is_coding=False)
    request.cls.effect_checker = CodingEffectChecker()
...
class IntronPositiveStrandTest(unittest.TestCase):
    def test_insertion_3_prime_side(self):
        ...
        self.assertEqual(effect.effect, "non-coding")
