...
from ..attributes_query_builder import (
    is_token,
    and_node,
    or_node,
    is_tree,
...
from ..attributes_query import (
    PARSER as attributes_query_parser,
...
@pytest.mark.parametrize("expr", ["someandother",])
...
@pytest.mark.parametrize(
    "expr", ["some and other", "some    and          other"]
)
...
@pytest.mark.parametrize(
    "expr,expected_missing_node",
    [["some and other", "logical_and"], ["some or other", "logical_or"]],
)
def test_ambiguity_with_binary_operations_is_resolved_correctly(
    ...
    assert len(tree.children) == 2
    assert any(
        tree_ambiguity.find_pred(lambda x: x.data == expected_missing_node)
    )
...
@pytest.mark.parametrize(
    "expr,expected_missing_node",
    [["someandother", "logical_and"], ["someorother", "logical_or"]],
)
...
def test_can_filter_not(transformer_matcher, expr, output):
    assert parse_and_transform(transformer_matcher, "not some", expr) == output
...
def test_can_filter_and(transformer_matcher, expr, output):
    assert (
        parse_and_transform(transformer_matcher, "some and other", expr)
        == output
    )
...
def test_can_filter_or(transformer_matcher, expr, output):
    assert (
        parse_and_transform(transformer_matcher, "some or other", expr)
        == output
    )
...
def test_can_filter_simple_all(transformer_matcher, expr, output):
    assert (
        parse_and_transform(transformer_matcher, "all(some, other)", expr)
        == output
    )
...
def test_can_filter_simple_any(transformer_matcher, expr, output):
    ...
    assert (
        parse_and_transform(transformer_matcher, "any(some, other)", expr)
        == output
    )
...
def test_can_filter_simple_eq(transformer_matcher, expr, output):
    assert (
        parse_and_transform(transformer_matcher, "eq(some, other)", expr)
        == output
    )
...
@pytest.mark.parametrize(
    "expr,output",
    [[[1], True], [[1, 2, 3, 4], True], [[2, 3, 4], False], [[], False]],
)
def test_token_simple_transformer_works(
    ...
    assert parse_and_transform(matcher, "some", expr) == output


@pytest.mark.parametrize(
    "expr,output",
    [[[1], False], [[1, 2, 3, 4], True], [[2, 3, 4], False], [[], False]],
)
def test_token_and_transformer(transformer_matcher_class, expr, output):
    token_map = {"some": 1, "other": 2}

    matcher = transformer_matcher_class(token_converter=lambda x: token_map[x])

    assert parse_and_transform(matcher, "some and other", expr,) == output
...
def test_token_eq_transformer(transformer_matcher_class, expr, output):
    ...
    assert parse_and_transform(matcher, "eq(some, other)", expr,) == output
...
@pytest.mark.parametrize(
    "node_constructor", [or_node, and_node, eq_node, any_node, all_node]
)
