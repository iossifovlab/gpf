...
class ParquetLoader(VariantsLoader):
    ...
    def _table_iterator(self, table, filename):
        ...
        for sv_index in summary_variants_indexes:
            records = []
            summary_variant_rows = df[df["summary_variant_index"] == sv_index]
            for index, row in summary_variant_rows.iterrows():
                if any(
                    r["allele_index"] == row["allele_index"] for r in records
                ):
                    continue
                ...
                record_dict = {
                    ...
                    "summary_variant_index": row["summary_variant_index"],
    ...
    def full_variants_iterator(self):
        for filename in self._get_filenames_to_open():
            for summary_variant, genotype in self._parquet_file_iterator(
                filename
            ):
                family_variants = []
