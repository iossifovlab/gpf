...
def test_layout_positions(layout_from_family2):
    ...
    assert (
        layout_from_family2.positions[0][0].individual.member.person_id
        == "mom2"
    )
    ...
    assert (
        layout_from_family2.positions[0][1].individual.member.person_id
        == "dad2"
    )
    ...
    assert (
        layout_from_family2.positions[1][0].individual.member.person_id
        == "id2"
    )
...
def test_layout__individuals_by_rank(layout_from_family2):
    ...
    assert (
        layout_from_family2._individuals_by_rank[0][0].member.person_id
        == "mom2"
    )
    assert (
        layout_from_family2._individuals_by_rank[0][1].member.person_id
        == "dad2"
    )

    assert len(layout_from_family2._individuals_by_rank[1]) == 1
    assert (
        layout_from_family2._individuals_by_rank[1][0].member.person_id
        == "id2"
    )
...
def test_layout__id_to_position(layout_from_family2):
    ...
    assert (
        layout_from_family2._id_to_position[
...
def test_layout_id_to_position(layout_from_family2):
    ...
    assert (
        layout_from_family2.id_to_position["mom2"].individual.member.person_id
        == "mom2"
    )
    assert (
        layout_from_family2.id_to_position["dad2"].individual.member.person_id
        == "dad2"
    )
    assert (
        layout_from_family2.id_to_position["id2"].individual.member.person_id
        == "id2"
    )
