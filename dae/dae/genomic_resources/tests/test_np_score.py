...
def test_np_score_aggregation() -> None:
    ...
    assert score.fetch_scores_agg(
        "1", 14, 18, [AlleleScoreQuery("cadd_raw")]) == \
        [(2 * 0.04 + 2 * 0.05) / 4.]

    assert score.fetch_scores_agg(
        "1", 14, 18, [AlleleScoreQuery("cadd_raw", "max")]) == \
        [0.05]

    assert score.fetch_scores_agg(
        "1", 14, 18, [AlleleScoreQuery("cadd_test")]) == \
        [3.0]

    assert score.fetch_scores_agg(
        "1", 14, 18, [AlleleScoreQuery("cadd_test", "min")]) == \
        [1.5]

    assert score.fetch_scores_agg(
        "1", 14, 18, [AlleleScoreQuery("cadd_test", "min", "min")]) == \
        [0]
