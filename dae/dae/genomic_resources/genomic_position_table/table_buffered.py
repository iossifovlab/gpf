...
class BufferedTable(GenomicPositionTable):
    ...
    def __init__(self, table: GenomicPositionTable, fetch_size: int = 2000):
        super().__init__(table.genomic_resource, table.definition)
        self._table = table
        self._buffer: list[LineBase] = []
    ...
    def _fill(self, chrom: str, start: int, stop: int) -> None:
        ...
        stop = min(self.get_chromosome_length(chrom), stop)
        range_start = start
        range_stop = min(stop, range_start + self.fetch_size)

        while not (res := list(
            self._table.get_records_in_region(chrom, range_start, range_stop))
        ) and range_stop <= stop:
        ...
        if res:
            self._buffer_region = Region(
                chrom,
                self._buffer[0].pos_begin,
                self._buffer[-1].pos_end)
