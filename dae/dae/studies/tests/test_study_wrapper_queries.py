...
@pytest.mark.parametrize(
    "variant_type,count", [(["ins"], 1), (["sub"], 1), (["del"], 1)], ids=repr
)
def test_query_variant_type_variants(
    variant_type, count, quads_variant_types_wrapper
):
    variants = list(
        quads_variant_types_wrapper.query_variants(variant_type=variant_type)
    )
...
@pytest.mark.parametrize(
    "effect_types,count", [(["Intergenic"], 3), (["CNV"], 0)]
)
...
def test_query_present_in_role(option, count, quads_in_child_wrapper):
    variants = list(
        quads_in_child_wrapper.query_variants(presentInRole=option)
    )
...
def test_query_present_in_role_compared_to_raw(
    ...
    vs = quads_f1_wrapper.studies[0].query_variants(roles=raw_query)
    vs = list(vs)

    variants = list(quads_f1_wrapper.query_variants(presentInRole=option))
    assert len(vs) == len(variants)
...
def test_query_min_alt_frequency(option, count, quads_in_child_wrapper):
    variants = list(
        quads_in_child_wrapper.query_variants(minAltFrequencyPercent=option)
    )
...
def test_query_max_alt_frequency(option, count, quads_in_child_wrapper):
    variants = list(
        quads_in_child_wrapper.query_variants(maxAltFrequencyPercent=option)
    )
...
def test_query_min_max_alt_frequency(
    ...
    variants = list(
        quads_in_child_wrapper.query_variants(
            minAltFrequencyPercent=minFreq, maxAltFrequencyPercent=maxFreq
        )
    )
...
def test_query_gene_weights(geneWeights, count, quads_f2_wrapper):
    variants = list(quads_f2_wrapper.query_variants(geneWeights=geneWeights))
    ...
    [
        ...
        (
            [
                ...
                {"metric": "score2", "rangeStart": 50.0, "rangeEnd": 150.0},
            ],
            1,
        ),
        (
            [
                {"metric": "score1", "rangeStart": 2.0, "rangeEnd": 7.0},
                {"metric": "score2", "rangeStart": 50.0, "rangeEnd": 75.0},
            ],
            0,
        ),
    ],
...
def test_query_genomic_scores(genomicScores, count, quads_f2_wrapper):
    variants = list(
        quads_f2_wrapper.query_variants(genomicScores=genomicScores)
    )
...
def test_query_people_group(people_group, count, quads_in_child_wrapper):
    variants = list(
        quads_in_child_wrapper.query_variants(peopleGroup=people_group)
    )
...
def test_transform_present_in_child(
    option, raw_query, quads_f1_wrapper
):
    vs = quads_f1_wrapper.genotype_data_study.query_variants(roles=raw_query)
    vs = list(vs)
