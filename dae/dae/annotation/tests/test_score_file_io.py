...
from dae.annotation.tools.score_file_io import (
    ScoreFile,
    TabixAccess,
    LineAdapter,
    LineBufferAdapter,
...
def test_regions_intersecting():
    ...
    for region_pair in regions:
        assert LineBufferAdapter.regions_intersect(
            region_pair[0][0],
            region_pair[0][1],
            region_pair[1][0],
            region_pair[1][1],
...
def test_regions_non_intersecting():
    ...
    for region_pair in regions:
        assert not LineBufferAdapter.regions_intersect(
            region_pair[0][0],
            region_pair[0][1],
            region_pair[1][0],
            region_pair[1][1],
...
def test_load_config():
    ...
    config_filename = relative_to_this_test_folder(
        "fixtures/sample_score_config.conf"
    )
...
def test_tabix_sequential_access_reset_backward(score_file, mocker):
    ...
    for check_pos in [20005, 20001]:
        ...
        assert len(res) == 1
        assert res[0][1] <= check_pos and res[0][2] >= check_pos
...
def test_tabix_sequential_access_reset_different_chrom(score_file, mocker):
    ...
    for chrom, check_pos in [("1", 20000), ("2", 20001)]:
        res = score_file.accessor._fetch_sequential(
            chrom, check_pos, check_pos
        )
        assert len(res) == 1
        assert res[0][1] <= check_pos and res[0][2] >= check_pos
...
def test_tabix_sequential_access_na_values(
    ...
    assert res[0][1] <= pos_start and res[count - 1][2] >= pos_end
