...
class FilterFamilies(object):
    ...
    def handle(self):
        print("Working with summary filename: {}".format(
        ...
        out_tm_filename = os.path.join(
            self.outdir,
            '{}-TOOMANY.txt'.format(self.name)
        )
        print("Storing result into: {} and {}".format(
            out_filename,
            out_tm_filename))

        with gzip.open(self.summary_filename, 'r') as fh, \
                gzip.open(self.tm_filename, 'r') as tmfh, \
                open(out_filename, 'w') as out, \
                open(out_tm_filename, 'w') as out_tm:
