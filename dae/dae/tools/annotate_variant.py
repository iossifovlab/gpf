...
def process( argv=sys.argv[1:] ):
   ...
   parser = optparse.OptionParser(
       ...
       description=desc, add_help_option=False)
   parser.add_option(
   ...
   parser.add_option(
       ...
       action='store')
   parser.add_option(
       '-r', help='reference allele (FOR SUBSTITUTIONS ONLY) column number/name',
       action='store')
   parser.add_option(
   ...
   parser.add_option(
       '--Traw', help='outside gene models file path', type='string',
       action='store')
   parser.add_option(
       '--TrawFormat',
   ...
   if not opts.help and ('DAE_DB_DIR' not in os.environ) and (not opts.Graw or not opts.Traw):
   ...
   argColumnNs = [
       chrCol, posCol, locCol, varCol, refCol, altCol, lengthCol,
       seqCol, typeCol]
