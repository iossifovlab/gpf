...
def generate_histograms(mocker, score_files, histogram_info):
    mocker.patch(
        "pandas.read_csv",
        side_effect=lambda _, usecols, sep, header: score_files.pop(0),
    )
...
def generate_histograms_by_chunks(
    ...
    mocker.patch(
        "pandas.read_csv",
        side_effect=lambda _, usecols, sep, header, chunksize, low_memory: score_files_by_chunks.pop(
            0
        ),
    )
    mocker.patch("matplotlib.pyplot.savefig", side_effect=None)
...
def generate_histograms_with_start_end(
    ...
    mocker.patch(
        "pandas.read_csv",
        side_effect=lambda _, usecols, sep, header, chunksize, low_memory: score_files_with_start_end.pop(
            0
        ),
    )
    mocker.patch("matplotlib.pyplot.savefig", side_effect=None)
...
def test_generate_histogram_with_start_end(
    mocker, generate_histograms_with_start_end, expected_output_with_start_end
):
    generate_histograms_with_start_end.generate_scores_histograms()

    assert (
        output_with_start_end[0].getvalue()
        == expected_output_with_start_end[0]
    )
    assert (
        output_with_start_end[1].getvalue()
...
def test_get_empty_bars_and_bins(
    ...
    empty_bars, empty_bins = generate_histograms.get_empty_bars_and_bins(
        histogram_info_1
    )
...
def test_fill_bars(
    generate_histograms, histogram_info_1, values, bars, bins, expected_bars
):
    histogram_info_1.bin_range = (1.0, 6.0)
