...
class PipelineConfigWrapper(VariantAnnotatorConfig):
    ...
    def __init__(
        self, name, annotator_name, options, columns_config, virtuals
    ):
        super(PipelineConfigWrapper, self).__init__(
            name, annotator_name, options, columns_config, virtuals
        )
    ...
    def build(cls, config_file):
        ...
        configuration = AnnotationConfigParser.read_and_parse_file_configuration(
    ...
    def _collect_score_groups(self):
        ...
        if self.non_freq_scores:
            for score_name in self.non_freq_scores:
                identifier = PipelineConfigWrapper.get_string_identifier(
                    score_name
                )
                group = PipelineConfigWrapper.collect_similar(
                    identifier, self.non_freq_scores
                )
                if len(group) > 1:
    ...
    def score_dirs(self):
        for section in self.pipeline_sections:
            if (
                section.annotator_name
                == "score_annotator.PositionMultiScoreAnnotator"
            ):
                for score in section.output_columns:
                    yield section.options.scores_directory + "/" + score
            elif (
                "score_annotator" in section.annotator_name
                or "frequency_annotator" in section.annotator_name
            ):
...
class ConfigGenerator(object):
    ...
    def generate_default_config(self):
        ...
        for score in self.pipeline_config.all_scores:
            option = generated_config_dict["genotypeBrowser"][
                "genotype"
            ].setdefault(score, {})
        ...
        for group in self.pipeline_config.score_groups:
            ...
            for score_name in self.pipeline_config.score_groups[group]:
                ...
                slots.append(
                    self.template_formatted_score.format(
                        score=score_name, name=browser_name
                    )
                )

            option["slots"] = ConfigGenerator.group_and_format(slots, width=1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate various configurations "
        "from an annotation config."
    )
    parser.add_argument("config", help="annotation config to use")
    parser.add_argument(
        "--generate", help="type of config to generate", action="append"
    )
    parser.add_argument(
        "--use-name",
        help=(
            "use a specific name in the genotype browser\n"
            "for the given score\n"
        ),
