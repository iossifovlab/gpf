    ...
    for directory in os.walk(scores_dir):
        if os.path.basename(directory[0]) == "histograms":
            for file_ in directory[2]:
                shutil.copy2(
                    os.path.join(directory[0], file_),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Export histograms from a genomic"
    ...
    parser.add_argument(
        "output_dir", help="directory to export histograms into"
    )
    group.add_argument(
        "--annotation_config", help="annotation pipeline config to use"
    )
    group.add_argument(
        "--genomic_scores_repo", help="genomic scores repository to use"
    )
