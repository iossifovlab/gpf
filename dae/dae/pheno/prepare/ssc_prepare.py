...
def load_and_join():
    ...
    individuals_v15_filename = os.path.join(
        data_dev_dir, "pheno/15", "Individuals_by_Distribution_v15.csv"
    )

    individuals_v14_filename = os.path.join(
        data_dev_dir, "pheno/14", "individual.csv"
    )

    persons_filename = os.path.join(data_dev_dir, "pheno/15", "persons.csv")

    individuals_v14_age_of_assessment_filename = os.path.join(
        data_dev_dir, "pheno/14", "ssc_age_at_assessment.csv"
    )
    ...
    individuals_df = pd.read_csv(
        persons_filename,
        sep=",",
        dtype={"family_id": object, "person_id": object,},
    )
    ...
    persons_14_age_df = pd.read_csv(
        individuals_v14_age_of_assessment_filename, sep=","
    )
...
def load_instrument(instrument_name):
    ...
    ssc_core_descriptive_filenames = [
        os.path.join(
            pheno_ssc_15_dir, measure_dir, "{}.csv".format(instrument_name)
        )
...
def infer_race(persons_df):
    ...
    persons_df = persons_df.join(
        commonly_used_df, on="personId", rsuffix="_commonly_used"
    )
    return persons_df
...
def build_race(race, race1, race2):
    ...
    if (
        race1 is None
        or race2 is None
        or race1 == "not-specified"
        or race2 == "not-specified"
        or race1 == ""
        or race2 == ""
        or isnan(race1)
        or isnan(race2)
...
def build_pedigree(persons_df):
    ...
    for _, family_df in grouped:
        ...
        for _index, row in family_df.iterrows():
            person = {k: row[k] for k in list(row.keys())}
            ...
            if row.role not in set([Role.mom.name, Role.dad.name]):
                ...
                person["race"] = build_race(
                    row.race, mom.race_parents, dad.race_parents
                )
    ...
    ped_df = pd.DataFrame(data=result, columns=COLUMNS,)
    return ped_df
