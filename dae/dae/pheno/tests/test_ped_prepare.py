...
@pytest.mark.parametrize('instrument_sel', [
    ([0, 0]),
    ([0, 1])
])
def test_ped_prepare_variable(test_config, temp_dbfile,
                              instrument_files, instrument_sel, fake_ped_file):
    test_config.person.role.mapping = 'INTERNAL'
...
def test_ped_prepare_variable_with_descriptions(test_config, temp_dbfile,
                                                instrument_files,
                                                fake_ped_file,
                                                valid_descriptions):
    test_config.person.role.mapping = 'INTERNAL'
