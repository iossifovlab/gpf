...
def create_sqlite_phenodb(dbfile: str, browser_dbfile: str) -> PhenoDb:
...
def create_duckdb_dbfile(dbfile: str, sqlite_db: PhenoDb) -> dict[str, Any]:
    ...
    measure_t = Table(
        ...
        Column("measure_type", Enum(MeasureType)),
...
def import_sqlite_into_duckdb(
    ...
    con.sql(
        "INSERT INTO variable_browser SELECT * FROM "
        f"sqlite_scan('{sqlite_browser_dbfile}', 'variable_browser')"
    )
    con.sql(
    ...
    for tbl_name in tables["instrument_values"].keys():
        con.sql(
            f"INSERT INTO {tbl_name}_measure_values "
