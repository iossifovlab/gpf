...
def test_init_pheno_df(fake_pheno_db):
    ...
    assert set(pheno_tool.pheno_df) == {'person_id', 'family_id',
                                        'role', 'status', 'sex',
...
def test_init_non_continuous_or_ordinal_measure(fake_pheno_db):
    with pytest.raises(AssertionError):
        PhenoTool(fake_pheno_db, 'i1.m5')  # categorical
    with pytest.raises(AssertionError):
        PhenoTool(fake_pheno_db, 'i1.m9')  # raw
...
def test_init_normalize_measures(fake_pheno_db):
    ...
    for measure_id in norm_measures:
        assert fake_pheno_db.has_measure(measure_id)


@pytest.mark.parametrize('measure_name', [('m4'), ('m5'), ('m6')])
def test_init_normalize_measures_non_continuous(fake_pheno_db, measure_name):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    norm_measures = [
        {'measure_name': '??', 'instrument_name': ''},
...
def test_get_normalize_measure_id_non_dict_measure(fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    with pytest.raises(AssertionError):
...
def test_get_normalize_measure_id_measure_dict_no_keys(fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    with pytest.raises(AssertionError):
...
def test_get_normalize_measure_id_no_instrument_name(fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
...
def test_get_normalize_measure_id_with_instrument_name(fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    measure_id = pheno_tool._get_normalize_measure_id(
        {'measure_name': 'm7', 'instrument_name': 'i1'})
...
@pytest.mark.parametrize('measure_name,instrument_name', [
    ...
    ('??', '??'),
    ('??', None)])
def test_get_normalize_measure_id_non_existent(fake_pheno_db, measure_name,
                                               instrument_name):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    measure_id = pheno_tool._get_normalize_measure_id(
...
def test_join_pheno_df_with_variants():
    pheno_df = pd.DataFrame([{'person_id': 112233, 'measure_value': 10},
    ...
    expected = pd.DataFrame([{'person_id': 112233,
                              'measure_value': 10,
                              'variant_count': 1.0},
                             {'person_id': 445566,
                              'measure_value': 20,
...
def test_normalize_df():
    ...
    assert normalized['normalized'][0] == \
        pytest.approx(-0.000299, abs=1e-4)  # FIXME:
    assert normalized['normalized'][1] == \
        pytest.approx(0.000488, abs=1e-2)  # FIXME:


def test_normalize_df_no_normalize_by():
    pheno_df = pd.DataFrame([{'person_id': 112233, 'i1.m1': 10},
                             {'person_id': 445566, 'i1.m1': 20}],
                            columns=['person_id', 'i1.m1'])
    expected = pd.DataFrame([{'person_id': 112233, 'i1.m1': 10,
                              'normalized': 10},
                             {'person_id': 445566, 'i1.m1': 20,
                              'normalized': 20}],
...
def test_calc(mocker, fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    variants = Counter({
        'f4.p1': 1, 'f5.p1': 1,
        'f7.p1': 1, 'f10.p1': 1,
    ...
    call_arg_df, call_arg_sex_split = \
        pheno_tool._calc_stats.call_args_list[0][0]
    assert merged_df.equals(call_arg_df)
    assert call_arg_sex_split is None


def test_calc_split_by_sex(mocker, fake_pheno_db):
    pheno_tool = PhenoTool(fake_pheno_db, 'i1.m1')
    variants = Counter({
        'f4.p1': 1, 'f5.p1': 1,
        'f7.p1': 1, 'f10.p1': 1,
