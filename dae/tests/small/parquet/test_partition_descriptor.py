...
def test_parse_toml_partition_description(tmp_path: pathlib.Path) -> None:
    ...
    assert len(pdesc.coding_effect_types) == 3


def test_parse_toml_partition_description_default_coding_effect_types(
    ...
    assert pdesc.family_bin_size == 2
    assert pdesc.rare_boundary == 50
    assert len(pdesc.coding_effect_types) == 16
...
def test_parse_yaml_partition_description_default_conding_effect_types(
    tmp_path: pathlib.Path,
) -> None:
    setup_directories(
        tmp_path / "partition_description.yaml",
